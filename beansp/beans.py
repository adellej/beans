...
# Parameter vector theta={}'''.format(file, time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        if self.numburstsobs > 0:
            ...
            _matched = ['	'+str(i in full_model['imatch']) for i in range(len(full_model['time']))]
        else:
            _fluene, _alphae = 0.01, 1.0 # dummy values
            _matched = [''] * len(full_model['time'])
        ...
        if self.train:
            ...
            for i in range(len(self.bstart)):
                print(_line_fmt.format(
                    ...
                    self.pfluxe[i],self.tdel[i],self.tdele[i]), file=f)

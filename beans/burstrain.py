...
#  i0=min([n_elements(a)-1,max(where(t1 gt tobs))])
    ...
    mdot0=(0.67/8.8)*(a[i0]+b[i0]*t1)*r1 # Initial guess at mean mdot (linear)

# print,base,x_0,z,mdot0
    tmp = settle(base,z, x_0,mdot0, cfac, mass, radius,binpath)
    if direction == 1:
        mdot=(0.67/8.8)*mean_flux(t1,t1+tmp.tdel/24.,tobs,a,b)*r1
    else:
        mdot=(0.67/8.8)*mean_flux(t1-tmp.tdel/24.,t1,tobs,a,b)*r1
...
#    mdot0=mdot
        ...
        tmp = settle(base,z, x_0,mdot, cfac, mass, radius)
...
#      mdot=mdot0+(mdot-mdot0)*randomu(seed)
            e = random.random()
#      mdot=mdot0+(mdot-mdot0)*randomu(seed)
            mdot = mdot_hist[len(mdot_hist)-1]*(1.-e)+mdot*e
        #if dbg:
...
# print,abs(mdot-mdot0)/mdot
    ...
    if mdot < minmdot or mdot > maxmdot and run == 0:
        ...
        # create array
    result = np.recarray((1,),dtype=[('t2', np.float64), ('e_b', np.float64), ('alpha', np.float64)])
    # assign elements
    result.t2 = t1+direction*tmp.tdel/24.
...
def generate_burst_train(base,z,x_0,r1,r2,r3,mass, radius, bstart,pflux, pfluxe, tobs,numburstssim,run=run,double=double,debug=debug):
...
# Here we introduce recurrence time corrections since the accretion rate is not flat over the extrapolated time, resulting in the recurrence time being underestimated by settle. Correction factors are from Zac, calculated using KEPLER
        ...
        if i == 0: # This is observed burst at 1.89
            cfac1 = 1.02041
            cfac2 = 1.02041
        if i == 1: # to the right this is 3rd observed burst, to left it is predicted burst
            cfac1 = 1.00
            cfac2 = 1.1905
        if i == 2: # to the right this is 4th observed burst, to left is predicted burst
            cfac1 = 1.00
            cfac2 = 1.2346
        if i == 3: # to the right this is final predicted burst, to the left is first observed burst (note that cfac = 1.25 is estimated interpolation)
            cfac1 = 1.00
            cfac2 = 1.25
...
# Find the time for the *previous* burst in the train, t2
            ...
            result2 = next_burst(base,z,x_0,sbt,tobs,a,b,r1,cfac1,mass,radius,binpath,direction=-1,run=run,debug=debug)

# Also find the time for the *next* burst in the train, t3

            result3 = next_burst(base,z,x_0,sbt,tobs,a,b,r1,cfac2,mass,radius,binpath,direction=1,run=run,debug=debug)

        else:

# Find the time for the *previous* burst in the train, t2
            result2 = next_burst(base,z,x_0,stime[0],tobs,a,b,r1,cfac1,mass,radius,binpath,direction=-1,run=run,debug=debug)

# Also find the time for the *next* burst in the train, t3

            result3 = next_burst(base,z,x_0,stime[len(stime)-1],tobs,a,b,r1,cfac2,mass, radius,binpath,direction=1,run=run,debug=debug)
